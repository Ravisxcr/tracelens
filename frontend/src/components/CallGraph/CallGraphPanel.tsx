...
export const CallGraphPanel: React.FC = () => {
  ...
  return (
    ...
    >
      {/* Panel Header */}
      <div className="h-9.5 bg-slate-100/80 dark:bg-[#1f1f22] border-b border-slate-200 dark:border-[#2b2b2b] px-3 flex items-center justify-between">
        <div className="flex items-center space-x-2 truncate">
          <GitGraph className="w-4 h-4 text-blue-500 dark:text-blue-400 shrink-0" />
          <span className="font-bold text-xs uppercase tracking-wider text-slate-700 dark:text-[#cccccc]">
            {isGraphFullScreen ? 'Full Screen Execution Graph' : 'Execution Graph'}
          </span>
          {callGraphSymbol && (
            <span className="font-mono text-[10.5px] bg-blue-50 dark:bg-blue-950/50 text-blue-700 dark:text-blue-300 px-2 py-0.5 rounded border border-blue-200 dark:border-blue-800/50 truncate max-w-[180px]">
              {callGraphSymbol}
            </span>
          )}
        </div>
        ...
        <div className="flex items-center space-x-2">
          ...
          <div className="relative flex items-center">
            <Search className="w-3 h-3 absolute left-2 text-slate-400 dark:text-[#666666] pointer-events-none" />
            <input
              type="text"
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
              placeholder="Filter graph..."
              className="h-6 pl-6 pr-5 bg-white dark:bg-[#26262a] border border-slate-200 dark:border-[#3a3a3e] rounded text-[11px] text-slate-800 dark:text-[#dddddd] placeholder:text-slate-400 dark:placeholder:text-[#666666] focus:outline-none focus:border-blue-500 w-28 transition-all focus:w-44"
            />
            {searchQuery && (
              <button
          ...
          <button
            ...
            className={`flex items-center space-x-1 px-2.5 py-1 rounded text-[11px] font-medium transition-colors cursor-pointer border ${
              isGraphFullScreen
          ...
          >
            {isGraphFullScreen ? (
              ...
              <>
                <Maximize2 className="w-3.5 h-3.5 text-blue-500 dark:text-blue-400" />
          ...
          <button
            ...
            className="p-1 hover:bg-slate-200 dark:hover:bg-[#2d2d30] rounded text-slate-400 dark:text-[#888888] hover:text-slate-800 dark:hover:text-white transition-colors cursor-pointer"
